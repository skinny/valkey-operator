...
func (r *ValkeySentinelReconciler) collectMonitorConfigs(ctx context.Context, s *valkeyiov1alpha1.ValkeySentinel) ([]monitoredValkey, map[string]string, error) {
	...
	for _, v := range matched {
		if v.Status.PrimaryEndpoint == nil || v.Status.PrimaryEndpoint.Host == "" {
		...
		if err != nil {
			logSwallowedError(log, err, "sentinel-auth secret unavailable; skipping", "valkey", v.Name)
			continue
		}
		monitors = append(monitors, monitoredValkey{
			Name:     v.Name,
			Host:     v.Status.PrimaryEndpoint.Host,
...
func (r *ValkeySentinelReconciler) reconcileMonitoring(ctx context.Context, s *valkeyiov1alpha1.ValkeySentinel, monitors []monitoredValkey) ([]string, bool, error) {
	...
	for _, c := range sentinelClients {
		...
		if err != nil {
			logSwallowedError(log, err, "SENTINEL MASTERS failed", "addr", c.Addr())
			continue
		}
		for _, n := range names {
			if !matchedNames[n] {
	...
	for name := range stale {
		for _, c := range sentinelClients {
			if err := c.Remove(ctx, name); err != nil {
				logSwallowedError(log, err, "SENTINEL REMOVE failed", "addr", c.Addr(), "master", name)
			}
		}
